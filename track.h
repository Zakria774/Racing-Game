...
public:
    ...
    void drawGraphics(sf::RenderWindow& window,
        ...
        for (int i = -1; i < height + 1; i++) {
            road.setPosition(sf::Vector2f(0, (i * cellSize) - roadOffset));  // Animate the road
            window.draw(road);
        }
        ...
        rightBoundary.setPosition(sf::Vector2f((width - 1) * cellSize, 0));
        ...
        for (int x = 0; x < width * cellSize; x += squareSize) {
            ...
            square.setPosition(sf::Vector2f(x, finishY));
        ...
        for (int y = 0; y < height * cellSize; y += cellSize * 2) {  // Create dashes
            ...
            line.setPosition(sf::Vector2f(middleX - 2, y - roadOffset));  // Position along middle column
        ...
        for (int y = 0; y < height * cellSize; y += cellSize * 2) {  // Create dashes
            sf::RectangleShape line;
            line.setSize(sf::Vector2f(5, cellSize));  // Make them thinner vertically
            line.setPosition(sf::Vector2f(middleX2 - 2, y - roadOffset));  // Position along the other middle column
            line.setFillColor(sf::Color::White);
            window.draw(line);
        }
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (r >= 0 && r < height && c > 0 && c < width - 1) {
                ...
                stripe.setPosition(sf::Vector2f(x + cellSize * 0.31f, y + cellSize * 0.47f));
                ...
                base.setPosition(sf::Vector2f(x + cellSize * 0.125f, y + cellSize * 0.80f));
        ...
        for (size_t i = 0; i < cars.size(); i++) {
            ...
            body.setPosition(sf::Vector2f(x + cellSize * 0.20f, y + cellSize * 0.10f));
            ...
            windowPart.setPosition(sf::Vector2f(x + cellSize * 0.325f, y + cellSize * 0.20f));
